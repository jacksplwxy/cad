<!-- 《如何用canvas实现一个富文本编辑器》： https://blog.csdn.net/sinat_33488770/article/details/131364266 -->
<!-- https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Drawing_text -->
<!-- https://www.canvasapi.cn/CanvasRenderingContext2D/measureText -->

<template>
  <div class="container">
    <div class="toolbar">
      <el-select v-model="fontFamily" size="small" style="width: 100px; margin-right: 6px" @change="exec('fontFamily')">
      ...
      <div class="btn italic" :class="{ active: isItalic }" @click="exec('italic')">
        <i></i>
      </div>
      <div class="btn underline" :class="{ active: isUnderline }" @click="exec('underline')">
        <i></i>
      </div>
      <div class="btn linethrough" :class="{ active: isLinethrough }" @click="exec('linethrough')">
        <i></i>
      </div>
      <el-color-picker v-model="color" show-alpha :predefine="predefineColors" size="small" @change="exec('color')" />
      <el-color-picker v-model="background" show-alpha :predefine="predefineColors" size="small" @change="exec('background')" />
      <el-select v-model="lineHeight" size="small" style="width: 100px; margin-right: 6px" @change="exec('lineHeight')">
...
let editor = null
onMounted(() => {
  editor = new RichText(editorContainer.value, data)
  editor.listeners.mousedown = (positionIndex) => {
    checkStyle(editor.data[positionIndex])
  }
  editor.listeners.rangeChange = (range) => {
    if (range.length > 0) {
      let { color: defaultColor, fontFamily: defaultFontFamily } = editor.options
...
.sizeAdd i {
  background-image: url('./imgs/size-add.svg');
}

.sizeMinus i {
  background-image: url('./imgs/size-minus.svg');
}
...
.linethrough i {
  background-image: url('./imgs/strikeout.svg');
}
</style>
